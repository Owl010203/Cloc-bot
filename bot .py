...
print(f"[STARTUP] BOT_TOKEN первые 20 символов: {BOT_TOKEN[:20]}")
print(f"[STARTUP] ADMIN_ID: {ADMIN_ID}")
print(f"[STARTUP] DATA_FILE будет: {os.getenv('DATA_DIR', '/app/data')}/data.json")
...
def calc_next_period_dates(after_date: str = None):
    ...
    if ref.day <= 5:
        ...
        start = date(y, m, 21)
        end = date(y + 1, 1, 5) if m == 12 else date(y, m + 1, 5)
    else:
        if m == 12:
            start = date(y + 1, 1, 6)
            end = date(y + 1, 1, 20)
